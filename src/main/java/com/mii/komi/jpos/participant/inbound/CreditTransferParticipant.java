 ...
 * @author vinch
 */
public class CreditTransferParticipant implements TransactionParticipant, BaseInboundParticipant {
    ...
    public int prepare(long id, Serializable context) {
        ...
        CreditTransferInboundRequest creditTransferRequest = (CreditTransferInboundRequest) ctx.get(Constants.HTTP_REQUEST);
        try {
            ISOMsg isoMsg = buildRequestMsg(creditTransferRequest);
    ...
    public void commit(long id, Serializable context) {
        ...
        CreditTransferInboundRequest creditTransferInboundRequest = (CreditTransferInboundRequest) ctx.get(Constants.HTTP_REQUEST);
        ResponseEntity<CreditTransferInboundResponse> restResponse = buildResponseMsg(creditTransferInboundRequest, isoMsg);
    ...
    public void abort(long id, Serializable context) {
        Context ctx = (Context) context;
        CreditTransferInboundRequest accountEnquiryRequest = (CreditTransferInboundRequest) ctx.get(Constants.HTTP_REQUEST);
        ISOMsg isoMsg = ctx.get(Constants.ISO_RESPONSE);
        ResponseEntity<CreditTransferInboundRequest> rr = null;
        if (isoMsg == null) {
    ...
    public ISOMsg buildRequestMsg(BaseInboundRequestDTO request) throws ISOException {
        CreditTransferInboundRequest debitRequest = (CreditTransferInboundRequest) request;
        ...
        ISOMsg isoMsg = ISO8583Service.buildFinancialMsg(CREDIT_TRANSFER_PC, request);
    ...
    public ResponseEntity buildFailedResponseMsg(BaseInboundRequestDTO request, ISOMsg rsp) {
        CreditTransferInboundResponse debitTransferRsp = new CreditTransferInboundResponse();
        CreditTransferInboundRequest originalRequest = (CreditTransferInboundRequest) request;
    ...
    @Override
    public ResponseEntity<CreditTransferInboundResponse> buildResponseMsg(BaseInboundRequestDTO request, ISOMsg isoMsg) {
